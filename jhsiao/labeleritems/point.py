...
class Point(Obj):
    ...
    @staticmethod
    def color(master, idn):
        return master.itemcget(idn, 'fill')

    @staticmethod
    def recolor(widget, idn, color):
    ...
    def snapto(widget, x, y):
        Obj._snapto(widget, x, y, Point.xy(widget, 'current'))

    @staticmethod
    @binds.bind('<Button-1>')
    def select(widget, x, y):
